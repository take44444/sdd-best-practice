...
# ---------------------------------------------------------------------------
# Source common.sh for json_escape, get_repo_root, has_git.
# ---------------------------------------------------------------------------
...
if [ "$_common_loaded" != "true" ]; then
    echo "Error: Could not locate common.sh." >&2
    exit 1
fi
...
if [ -n "${GIT_BRANCH_NAME:-}" ] && [ "$BRANCH_BYTE_LEN" -gt $MAX_BRANCH_LENGTH ]; then
    ...
    >&2 echo "[git-hook] Warning: Branch name exceeded GitHub's 244-byte limit"
    >&2 echo "[git-hook] Original: $ORIGINAL_BRANCH_NAME (${#ORIGINAL_BRANCH_NAME} bytes)"
    >&2 echo "[git-hook] Truncated to: $BRANCH_NAME (${#BRANCH_NAME} bytes)"
fi
...
if [ "$DRY_RUN" != true ]; then
    if [ "$HAS_GIT" = true ]; then
        ...
        >&2 echo "[git-hook] Warning: Git repository not detected; skipped branch creation for $BRANCH_NAME"
    fi
